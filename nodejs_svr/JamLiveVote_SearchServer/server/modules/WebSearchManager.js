...
class WebSearchEngine {
    ...
    searchDic( query, client ) {
        ...
        const wse = this;
        query = query.trim();
        var cached = this.getCachedSearchResult('naver_chinese', query);
        var cached_h = this.getCachedSearchResult('naver_chinese_h', query);
        var cached_e = this.getCachedSearchResult('naver_chinese_e', query);
            ...
            request.get(options, function (error, response, body) {
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    if( isArray(data) && data.length > 0 ) {
                        data = data.slice(0,4);
                        wse.setCachedSearchResult('naver_chinese', query, data);
                        wse.setCachedSearchResult('naver_chinese_h', query, hanja_data);
                        wse.setCachedSearchResult('naver_chinese_e', query, endic_data);
    ...
    searchDaum(query, client) {
        ...
        query = query.trim();
        var cached = this.getCachedSearchResult('google', query);
        if( cached ) {
            client.socket.emit(PROTOCOL, cached);
            return;
        }
    ...
    searchImage( query, client ) {
        ...
        var sType = 'image';
        var cached = this.getCachedSearchResult(sType, query);
        if( cached ) {
            client.socket.emit(PROTOCOL, cached);
            return;
        }
            ...
            request.get(options, function (error, response, body) {
                if (!error && response.statusCode == 200 && body) {
                    try {
                        ...
                        if( isArray(data) && data.length > 0 ) {
                            data = data.slice(0,8);
                            wse.setCachedSearchResult(sType, query, data);
                            client.socket.emit(PROTOCOL, data);
                        }
                        else {
                            client.socket.emit(PROTOCOL, []);
